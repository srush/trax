...
class TraxKerasLayer(tf.keras.layers.Layer):
  ...
  original Trax layer (which uses tensors to store weights), so creating
  `TraxKeraLayer` may double the memory usage. Mutations in those `tf.Variable`s
  won't affect the Trax layer's weights, but `TraxKeraLayer`'s forward function
  calls the Trax layer's forward function, which caches the weights in the Trax
  layer object, so a forward pass may change the weights stored in the original
  Trax layer.
